...
function criarPlanilhaProjetos() {
  var ss = SpreadsheetApp.create("Controle de Projetos — Saulo Zabot");
  var antigas = ss.getSheets();
  // 1. Cria abas de projeto PRIMEIRO para que as fórmulas da CAPA funcionem
  for (var i=0;i<CFG.projetos.length;i++) criarProjeto(ss,CFG.projetos[i],i===0);
  // 2. Remove abas padrão (Plan1, Sheet1, etc.)
  antigas.forEach(function(a){
    var n=a.getName();
    if(["Plan1","Sheet1","Página1","Planilha1"].indexOf(n)>=0){try{ss.deleteSheet(a);}catch(e){}}
  });
  // 3. Cria CAPA por último (inserida na posição 0) — fórmulas já encontram as abas
  criarCapa(ss);
  try { ss.setActiveSheet(ss.getSheetByName("🏠 CAPA")); } catch(e) {}
  var url = ss.getUrl();
  Logger.log("✅ Planilha criada com sucesso!");
  Logger.log("🔗 Acesse em: " + url);
  Logger.log("📋 Duplique qualquer aba 'Projeto X' para novos projetos.");
}
...
function criarCapa(ss){
  ...
  // ── MANUTENÇÃO MENSAL — RESUMO POR PROJETO ────────────────
  a.setRowHeight(r,34);
  mes(a,"B"+r+":H"+r,"🔧  MANUTENÇÃO MENSAL ESTIMADA — TODOS OS PROJETOS",{bg:CFG.CM,fg:CFG.AM,b:true,sz:13,al:"left",va:"middle"});r++;
  a.setRowHeight(r,22);
  mes(a,"B"+r+":H"+r,"Valores estimados mensais para manutenção de cada projeto em produção.",{bg:CFG.AP,fg:CFG.AM,sz:10,al:"left",va:"middle"});r++;
  hTab(a,r,["PROJETO","HORAS/MÊS","CUSTO HORAS","CRÉDITOS/MÊS","TOTAL MENSAL","—","—"],CFG.AM,CFG.BR);r++;

  for(var mi=0;mi<CFG.projetos.length;mi++){
    var mp=CFG.projetos[mi],mbg=mi%2===0?CFG.BR:CFG.CC;
    var mref="'"+mp.emoji+" "+mp.nome+"'";
    a.setRowHeight(r,30);
    cel(a,r,2,mp.emoji+"  "+mp.nome,{bg:mbg,fg:CFG.PT,b:mi===0,sz:11,al:"left",va:"middle",bd:true});
    var mfH=a.getRange(r,3);mfH.setFormula("=IFERROR("+mref+"!E183,0)");
    _o(mfH,{bg:mbg,fg:CFG.LE,b:true,sz:11,al:"right",va:"middle",fm:'0.0"h"',bd:true});
    var mfCH=a.getRange(r,4);mfCH.setFormula("=IFERROR("+mref+"!G183,0)");
    _o(mfCH,{bg:mbg,fg:CFG.PT,sz:11,al:"right",va:"middle",fm:fmtMoeda(),bd:true});
    var mfCr=a.getRange(r,5);mfCr.setFormula("=IFERROR("+mref+"!F183,0)");
    _o(mfCr,{bg:mbg,fg:CFG.RE,sz:11,al:"right",va:"middle",fm:fmtMoeda(),bd:true});
    var mfT=a.getRange(r,6);mfT.setFormula("=IFERROR("+mref+"!H183,0)");
    _o(mfT,{bg:mbg,fg:CFG.AM,b:true,sz:11,al:"right",va:"middle",fm:fmtMoeda(),bd:true});
    mes(a,"G"+r+":H"+r,"",{bg:mbg,bd:true});
    r++;
  }
  var mTotStart=r-CFG.projetos.length;
  a.setRowHeight(r,28);
  mes(a,"B"+r+":C"+r,"TOTAL GERAL / MÊS",{bg:CFG.AM,fg:CFG.BR,b:true,sz:10,al:"right",va:"middle",bd:true});
  var mfTH=a.getRange(r,3);mfTH.setFormula("=SUM(C"+mTotStart+":C"+(r-1)+")");
  _o(mfTH,{bg:CFG.AM,fg:CFG.BR,b:true,sz:11,al:"right",va:"middle",fm:'0.0"h"',bd:true});
  var mfTC=a.getRange(r,4);mfTC.setFormula("=SUM(D"+mTotStart+":D"+(r-1)+")");
  _o(mfTC,{bg:CFG.AM,fg:CFG.BR,b:true,sz:11,al:"right",va:"middle",fm:fmtMoeda(),bd:true});
  var mfTCr=a.getRange(r,5);mfTCr.setFormula("=SUM(E"+mTotStart+":E"+(r-1)+")");
  _o(mfTCr,{bg:CFG.AM,fg:CFG.BR,b:true,sz:11,al:"right",va:"middle",fm:fmtMoeda(),bd:true});
  var mfTT=a.getRange(r,6);mfTT.setFormula("=SUM(F"+mTotStart+":F"+(r-1)+")");
  _o(mfTT,{bg:CFG.AM,fg:CFG.BR,b:true,sz:11,al:"right",va:"middle",fm:fmtMoeda(),bd:true});
  mes(a,"G"+r+":H"+r,"",{bg:CFG.AM,bd:true});
  r++;
  sp(a,r,20);r++;
...
function criarProjeto(ss,proj,isLux){
  ...
  // ── L168-185: MANUTENÇÃO MENSAL ESTIMADA ──────────────────
  // Total fixo na linha 183 (referenciado pela CAPA)
  // E183=horas/mês  F183=créditos/mês  G183=custo horas  H183=total mensal
  sp(a,168,20);
  a.setRowHeight(169,32);
  mes(a,"B169:H169","🔧  MANUTENÇÃO MENSAL ESTIMADA  —  Horas & Créditos Recorrentes",{bg:CFG.LE,fg:CFG.BR,b:true,sz:12,al:"left",va:"middle"});
  a.setRowHeight(170,24);
  mes(a,"B170:H170","Estime os recursos mensais para manter este projeto em produção. O valor/hora é lido de C10.",{bg:CFG.LP,fg:CFG.LE,sz:10,al:"left",va:"middle"});
  hTab(a,171,["MÊS REF.","ITEM DE MANUTENÇÃO","TIPO","HORAS/MÊS","CRÉDITOS/MÊS","CUSTO HORAS","TOTAL MENSAL"],CFG.LP,CFG.LE);

  var mantEx=isLux?[
    ["Recorrente","Suporte a usuários","Horas",4,0],
    ["Recorrente","Atualizações de segurança","Horas",2,0],
    ["Recorrente","Firebase (Auth+Firestore+Storage)","Crédito",0,0],
    ["Recorrente","Vercel (Hosting/API)","Crédito",0,0],
    ["Recorrente","Monitoramento e logs","Horas",1,0],
    ["Recorrente","Google Play (atualizações)","Horas",2,0]
  ]:[];

  for(var mni=0;mni<10;mni++){
    var mnr=172+mni,mnbg=mni%2===0?CFG.BR:CFG.CC;
    a.setRowHeight(mnr,24);
    if(isLux&&mni<mantEx.length){
      var mne=mantEx[mni];
      cel(a,mnr,2,mne[0],{bg:mnbg,fg:CFG.PT,sz:10,al:"center",va:"middle",bd:true});
      cel(a,mnr,3,mne[1],{bg:mnbg,fg:CFG.PT,sz:10,al:"left",va:"middle",bd:true});
      cel(a,mnr,4,mne[2],{bg:mnbg,fg:CFG.PT,sz:10,al:"center",va:"middle",bd:true});
      cel(a,mnr,5,mne[3],{bg:mnbg,fg:CFG.LE,b:mne[3]>0,sz:10,al:"right",va:"middle",bd:true});
      cel(a,mnr,6,mne[4],{bg:mnbg,fg:CFG.RE,sz:10,al:"right",va:"middle",fm:fmtMoeda(),bd:true});
      var mnCH=a.getRange(mnr,7);mnCH.setFormula("=IF(E"+mnr+">0,E"+mnr+"*$C$10,0)");
      _o(mnCH,{bg:mnbg,fg:CFG.LE,sz:10,al:"right",va:"middle",fm:fmtMoeda(),bd:true});
      var mnTot=a.getRange(mnr,8);mnTot.setFormula("=F"+mnr+"+G"+mnr);
      _o(mnTot,{bg:mnbg,fg:CFG.AM,b:true,sz:10,al:"right",va:"middle",fm:fmtMoeda(),bd:true});
    } else {
      for(var mndc=2;mndc<=8;mndc++) cel(a,mnr,mndc,"",{bg:mnbg,fg:CFG.PT,sz:10,al:mndc>=5?"right":"left",va:"middle",bd:true});
      a.getRange(mnr,6).setNumberFormat(fmtMoeda());
      var mnCHf=a.getRange(mnr,7);mnCHf.setFormula("=IF(E"+mnr+">0,E"+mnr+"*$C$10,0)");
      _o(mnCHf,{bg:mnbg,fm:fmtMoeda()});
      var mnTf=a.getRange(mnr,8);mnTf.setFormula("=IF(OR(E"+mnr+">0,F"+mnr+">0),F"+mnr+"+G"+mnr+",\"\")");
      _o(mnTf,{bg:mnbg,fm:fmtMoeda()});
    }
  }
  sp(a,182,8);
  // ── TOTAL fixo na linha 183 (referenciado pela CAPA) ──────
  a.setRowHeight(183,28);
  mes(a,"B183:D183","TOTAL MENSAL ESTIMADO",{bg:CFG.LE,fg:CFG.BR,b:true,sz:10,al:"right",va:"middle",bd:true});
  var mnTE=a.getRange(183,5);mnTE.setFormula("=SUM(E172:E181)");
  _o(mnTE,{bg:CFG.LE,fg:CFG.BR,b:true,sz:12,al:"right",va:"middle",fm:'0.0"h"',bd:true});
  var mnTF=a.getRange(183,6);mnTF.setFormula("=SUM(F172:F181)");
  _o(mnTF,{bg:CFG.LE,fg:CFG.BR,b:true,sz:12,al:"right",va:"middle",fm:fmtMoeda(),bd:true});
  var mnTG=a.getRange(183,7);mnTG.setFormula("=SUM(G172:G181)");
  _o(mnTG,{bg:CFG.LE,fg:CFG.BR,b:true,sz:12,al:"right",va:"middle",fm:fmtMoeda(),bd:true});
  var mnTH=a.getRange(183,8);mnTH.setFormula("=SUM(H172:H181)");
  _o(mnTH,{bg:CFG.LE,fg:CFG.BR,b:true,sz:12,al:"right",va:"middle",fm:fmtMoeda(),bd:true});
  sp(a,184,20);
  a.setRowHeight(185,26);
  mes(a,"B185:H185","© "+new Date().getFullYear()+"  Saulo Zabot  •  "+proj.nome+"  •  Controle de Projetos v1.0",{bg:CFG.AE,fg:CFG.CZ,sz:9,al:"center",va:"middle"});
